...
def main(_):
    ...
    fcn16_fc_weights = [v for v in global_variable_list if v.name == "fcn16/downsampled_by_16_fc/weights:0"]
    fcn16_upsample_by_2_filter = [v for v in global_variable_list if "fcn16/upsample_by_2" in v.name and "sgd" not in v.name]
    stage_reinitializers['fcn16'] = tf.variables_initializer(fcn16_fc_weights + fcn16_upsample_by_2_filter)

    fcn8_fc_weights = [v for v in global_variable_list if v.name == "fcn8/downsampled_by_8_fc/weights:0"]
    fcn8_upsample_by_2_filter = [v for v in global_variable_list if "fcn8/upsample_by_2" in v.name and "sgd" not in v.name]
    stage_reinitializers['fcn8'] = tf.variables_initializer(fcn8_fc_weights + fcn8_upsample_by_2_filter)
    print(stage_reinitializers['fcn16'])
    print(stage_reinitializers['fcn8'])
