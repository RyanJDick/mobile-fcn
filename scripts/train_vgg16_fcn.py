...
def main(_):
    ...
    ############################
    is_training = tf.placeholder(tf.bool) # Controls whether dropout layers are enabled in VGG16 network
    ...
    ##################################
    if not cfg['restore_from_trained_checkpoint'] : # Load pretrained VGG-16 Imagenet weights
        variables_to_restore = slim.get_model_variables('vgg_16')
        init_vgg16_fn = slim.assign_from_checkpoint_fn(model_path=cfg['base_checkpoint_filename'], var_list=variables_to_restore)
    ...
    with tf.Session() as sess:
        ...
        if cfg['restore_from_trained_checkpoint'] :
            ...
            init_vgg16_fn(sess) # Load base checkpoint (ex: vgg16 weights before training FCN32 weights)
                ...
                while True: # Iterate over entire training set
                    ...
                    if train_count % cfg['train_steps_between_validation'] == 0:
                        ...
                        while True: # Iterate over entire validation set
                            try:
                                sess.run(iou_metric_updaters[stage], feed_dict={handle: validation_handle, is_training: False})
